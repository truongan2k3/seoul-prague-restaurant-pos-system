...
function ServerApp() {
  ...
  const reloadTables = useCallback(async () => {
    const { data, error } = await fetchTableSummaries();
    if (!error) setTables(mapTablesResponse(data));
  }, []);

  const reloadMenu = useCallback(async () => {
    const { data, error } = await loadMenuItemsResolved();
    if (!error && data) setMenuItems(data);
  }, []);

  const reloadCategories = useCallback(async () => {
    const { data, error } = await fetchCategories();
    if (!error) setCategories(mapCategoriesResponse(data));
  }, []);

  const reloadOrderItems = useCallback(async () => {
    const { data, error } = await fetchActiveOrderItems();
    if (!error) {
  ...
  const reloadAll = useCallback(async () => {
    await Promise.all([reloadTables(), reloadMenu(), reloadCategories(), reloadOrderItems()]);
  ...
  useEffect(() => {
    ...
    const unsubs = [
      subscribeToPostgresRowChanges(
        "server-tables-row-sync",
        { event: "*", schema: "public", table: "tables" },
        (payload) => setTables((prev) => applyTableRealtimeEvent(prev, payload)),
      ),
      subscribeToPostgresRowChanges(
        "server-order-items-row-sync",
        { event: "*", schema: "public", table: "order_items" },
        (payload) => setOrderItems((prev) => applyOrderItemRealtimeEvent(prev, payload)),
      ),
  ...
  }, [reloadAll, reloadMenu, reloadCategories]);
  ...
  const handleSend = async (orders: OrderItem[]) => {
    ...
    setIsSaving(true);
    try {
      const prepared = applyFulfillmentModeToNewOrders(orders, settings.kitchenFulfillmentMode);
    ...
    <div className="min-h-screen bg-zinc-100 dark:bg-zinc-950">
      ...
      <div className="grid grid-cols-2 gap-3 p-3 sm:grid-cols-3 sm:gap-3 md:grid-cols-4 md:gap-4 lg:grid-cols-6">
        {tables.map((table) => (
          ...
          >
            ...
            {table.status !== "empty" && floorItemsForTable(table.id).length > 0 && (
              <p className="mt-1 text-[10px] text-zinc-500 dark:text-zinc-400">
                {floorItemsForTable(table.id).length} item(s)
              </p>
            )}
          </button>
        ))}
      ...
      {orderModal && (
        <NewOrderModal
          ...
          existingOrders={floorItemsForTable(orderModal.table.id)}
