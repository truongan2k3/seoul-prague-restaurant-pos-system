        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100">
              {translate("menuItemCustomizations")}
            </h2>
          ...
          {customizedItems.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-100 rounded-lg border border-gray-100 dark:divide-gray-800 dark:border-gray-700">
              {customizedItems.map((item) => {
                ...
                const groupCount =
                  item.customizationConfig?.optionGroups?.length ?? libraryCount;
                return (
                  <li key={item.id} className="flex items-center gap-3 px-3 py-2.5">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {item.category} · {groupCount}{" "}
                        {translate("optionGroupChoices").toLowerCase()}
                        {libraryCount > 0
                          ? ` · ${libraryCount} ${translate("fromLibrary")}`
                          : ""}
                      </p>
                    </div>
                    <button
