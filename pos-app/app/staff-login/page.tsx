  ...
  return (
    <div className="mt-6 space-y-2" aria-hidden>
      {[0, 1, 2].map((index) => (
        <div
          key={index}
          className="h-[52px] animate-pulse rounded-xl border border-gray-200 bg-gray-100 dark:border-zinc-700 dark:bg-zinc-800"
        />
      ))}
    </div>
  );
...
function StaffLoginForm() {
  ...
  const loadRoster = useCallback(async () => {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      const existing = await resolveStaffLoginSessionAction();
      if (existing) {
        router.replace(nextPath);
        return;
      }

      const result = await prepareStaffLoginRosterAction();
      const active = (result.data ?? []).filter((member) => member.active && member.name.trim());

      if (active.length > 0 || result.error) {
        setRoster(active);
  ...
  if (authLoading || !session) {
    return (
      <>
        <StaffLoginSkeleton />
        <p className="mt-4 text-center text-sm text-gray-500">{translate("loading")}</p>
      </>
    );
  }
    ...
    <>
      <div className="mb-6 text-center">
        ...
        <h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-zinc-100">
          {session.businessName.trim() || translate("staffLoginTitle")}
        </h1>
      </div>
            ...
            <div className="space-y-3">
              ...
              {loadError === "unauthorized" && (
                <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">
                  {translate("staffLoginBusinessRequired")}
                </p>
              )}
              <button
                type="button"
                onClick={() => void loadRoster()}
                className="min-h-[44px] w-full rounded-xl border border-gray-300 bg-white text-sm font-semibold text-gray-800 hover:bg-gray-50 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:bg-zinc-800"
              >
                {translate("retry")}
              </button>
            </div>
